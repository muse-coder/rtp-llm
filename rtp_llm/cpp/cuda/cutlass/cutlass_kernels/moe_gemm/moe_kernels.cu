...
__launch_bounds__(TPB) __global__
    ...
    for (int ii = threadIdx.x; ii < num_cols; ii += TPB) {
        int64_t const idx = thread_row_offset + ii;
        float const val = isfinite(Z) ? (exp((static_cast<float>(input[idx]) - float_max)) * normalizing_factor) : 0.f;
        output[idx]     = val;
    }
}
...
__launch_bounds__(TPB) __global__ void moeTopK(float const*                    inputs_after_softmax,
    ...
    for (int k_idx = startk; k_idx < endk; ++k_idx) {
        ...
        for (int expert = threadIdx.x; expert < num_experts; expert += TPB) {
            ...
            if (k_idx == startk && abs(inp_kvp.value) < 1e-6) {
                ++s0;
            }
            if (k_idx == startk && inp_kvp.value < 0.f) {
                ++sm;
            }
    ...
    __syncthreads();
    if (threadIdx.x == 0) {
        int64_t cnt0 = 0, cnt1 = 0, cnt2 = 0, cnt3 = 0, cnt4 = 0;
        for (int64_t i = startk; i < endk; ++i) {
            int64_t const idx       = indice_offset + i;
            int const     expert_id = static_cast<int>(indices[idx]);
            if (expert_id == 0) {
                ++cnt0;
            }
            if (expert_id == 1) {
        ...
        if (cnt0 > 1 || cnt1 > 1 || cnt2 > 1 || cnt3 > 1 || cnt4 > 1) {
            printf(
                "fix_moe_topk: cnt0 = %ld, cnt1 = %ld, cnt2 = %ld, cnt3 = %ld, cnt4 = %ld, num_rows = %ld, block_row = %ld, num_experts = %d, s0 = %ld, sm = %ld, value = %f %f %f %f %f %f %f %f\n",
                cnt0,
                cnt1,
                cnt2,
                cnt3,
                ...
                s0,
                sm,
                output[indice_offset],
                output[indice_offset + 1],
                output[indice_offset + 2],
                output[indice_offset + 3],
                output[indice_offset + 4],
                output[indice_offset + 5],
                output[indice_offset + 6],
                output[indice_offset + 7]);
        }
    }
}
...
template<class T>
__global__ void
loraReorderKernel(T* output, T const* lora_result, int64_t const* num_valid_tokens_ptr, int64_t inter_size) {
...
__global__ void dequantFP8Kernel(OutputType*      output,
                                 InputType const* input,
                                 int64_t const*   num_valid_tokens_ptr,
                                 int64_t          inter_size,
                                 float const*     scale,
                                 bool             scale_is_dequant) {
    int64_t const tid   = threadIdx.x;
    int64_t const token = blockIdx.x;
    if (num_valid_tokens_ptr && token >= *num_valid_tokens_ptr) {
        return;
...
template<class OutputType, class InputType>
void dequantFP8(OutputType*      output,
                InputType const* input,
                int64_t const*   num_valid_tokens_ptr,
                int64_t          inter_size,
                int64_t          num_tokens,
...
__global__ void prepareFakeRouterBuffers(int*    unpermuted_source_rows,
                                         int*    unpermuted_expert_selection,
                                         int64_t num_tokens,
                                         int64_t k,
                                         int64_t num_experts,
...
void GemmProfilerBackend::prepare(int num_tokens, char* workspace, cudaStream_t stream) {
    ...
    for (int64_t i = 0; i < NUM_ROUTING_SAMPLES; i++) {
        ...
        sortAndScanSoftmaxOutput(unpermuted_expert_selection,
                                 unpermuted_source_rows,
                                 permuted_experts,
                                 dest_to_source,
...
void GemmProfilerBackend::runProfiler(int                 original_num_tokens,
                                      Config const&       tactic,
    ...
    if (mWType == nvinfer1::DataType::kINT8 || mWType == nvinfer1::DataType::kINT4) {
        ...
        quant_params = QuantParams::FP8(static_cast<float const*>(scale_1),
                                        static_cast<float const*>(scale_2),
                                        static_cast<float const*>(scale_3),
                                        static_cast<float const*>(scale_4));
    }
